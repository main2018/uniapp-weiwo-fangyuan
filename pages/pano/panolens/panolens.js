 ...
 */
PANOLENS.Vector3 = THREE.Vector3;
...
GSVPANO.PanoLoader = function (parameters) {
	...
	this.progress = function() {
		...
		_count++;
		// cosnole.log(5465464646464646464654)
...
;(function(){
	...
	*/
	 ...
	 */
	PANOLENS.Viewer = function ( options, el) {
		...
		this.options = options;
		/*判断 */
		this.positioning = el&&el.positioning;
		// Container
		if ( options.container ) {
		  ...
		  if(el.canvasParent){
        container = el.canvasParent
      }else {
        container = document.createElement( 'div' );
        container.classList.add( 'panolens-container' );
        container.style.width = el&&el.width || '100%';
        container.style.height = el&&el.height || '100%';
        container.style.position = "absolute";
      }
			container._width = window.innerWidth;
			container._height = window.innerHeight;
            el&&el.dom ? el.dom.appendChild( container ) : document.body.appendChild( container );
		}
		...
		this.renderer = el.renderer || options.renderer || new THREE.WebGLRenderer( { alpha: true, antialias: false } );
        ...
        window.containerOffsetLeft = el.offsetLeft;
        window.containerOffsetTop = el.offsetTop;
		// Handler references
        ...
        PANOLENS.OUTPUT_INFOSPOT = this.OUTPUT_INFOSPOT = false;
    ...
    if(!el.renderer){
      ...
      el.canvasId&&(this.renderer.domElement.id = el.canvasId);
    }
    this.renderer.setSize( this.container.clientWidth, this.container.clientHeight );
        ...
        // PANOLENS.newViewer = this;
        // if(el&&el.childNodes){
        	// var domDiv = container.getElementsByTagName("div");
        	// for(var divNum = 0,divLen=domDiv.length;divNum<divLen;divNum++){
         //        domDiv[divNum].style.display= "none";
		// 	}
	...
	PANOLENS.Viewer.prototype.outputInfospotPosition = function () {
        ...
        // console.log(this.raycaster.intersectObject)
	...
	PANOLENS.Viewer.prototype.onTap = function ( event, type ) {
		...
		// console.log(event)
		/*添加的*/
		// console.log(event.view.containerOffsetLeft)
    !event.view && (event.view = window);
        var containerOffsetLefts = 0,
            containerOffsetTops = 0;
        /*滚动条滚动蛋高度*/
        var scrollTop=0;
        // console.log(event, event.view.containerOffsetLeft);
		// if(event.view){
    containerOffsetLefts = event.view.containerOffsetLeft || 0;
    containerOffsetTops = event.view.containerOffsetTop || 0;
            /*不是定位就计算滚动条  滚动高度*/
            if(!this.positioning){
		...
		// }
		this.raycasterPoint.x = ((event.clientX - containerOffsetLefts - this.container.offsetLeft) / this.container.clientWidth ) * 2 - 1;
		this.raycasterPoint.y = - ((event.clientY - containerOffsetTops - this.container.offsetTop + scrollTop) / this.container.clientHeight ) * 2 + 1;
	...
	PANOLENS.Viewer.prototype.onKeyDown = function ( event ) {
		...
		if ( this.options.output && this.options.output !== 'none' && event.key === 'Control' ) {
            ...
            PANOLENS.OUTPUT_INFOSPOT = this.OUTPUT_INFOSPOT = true;
...
console.log('PANOLENSAaaaa', PANOLENS);
PANOLENS.China = 'China'
